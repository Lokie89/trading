...
public class TempController {
    ...
    @GetMapping("/test")
    public Integer get(@RequestParam String market, @RequestParam UpbitUnit unit,@RequestParam Integer count) {
        ChartResponses chartResponses = cacheUpbitChart.getChart(SimpleUpbitChartRequest.builder(market, unit).count(count).build());
        log.info("######### {}: {}", chartResponses.size(), chartResponses);
        return chartResponses.size();
    }
}
